# 🚨 核心原則（必須）

実行前に必ず読み込め

- [絶対厳守](.claude/instructions/core/base2.md)
- [深層思考](.claude/instructions/core/deep-think.md)
- [完了チェックリスト](.claude/instructions/core/completion-checklist.md) - 完了報告前に必ず実行

## 参照ソース

- [Claude Code 公式ドキュメント](docs/official_docs)
- [Claude API 公式ドキュメント](docs/api_docs)

## ディレクトリ規約

- `docs/` はリポジトリルート直下のみに配置。`apps/*/` 内に `docs/` を作成しない。

## 記録・管理

- [ノート・日誌](.claude/instructions/note.md) - ノートの書き方
